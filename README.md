...
Harness 仍不会启动、关闭或重启 Kimi、Claude、Pi 或 OMP。阶段 91 已于 2026-07-28 完成真实 GUI 验收：操作者只作启动和最终“通过 / 要求修改”两次决定，真实 `Pi → OMP → Pi` 自动串行完成后自动弹出最终决定页，最终链路为 `approved`；活动工具非空、宿主忙碌、状态漂移、事件链无效、结果不安全或审阅绑定漂移时都会失败关闭。
