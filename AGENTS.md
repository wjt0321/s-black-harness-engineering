...
2. `docs/90-codex-desktop-filtered-snapshot-host-integration-and-milestone-freeze.md`：Stage 30–32 design、实现、验收与 v0.14.0 冻结事实源。
3. `docs/89-codex-desktop-filtered-snapshot-consumer-implementation.md`：Stage 29 stdin consumer 实现与验收边界。
4. `docs/87-filtered-envelope-snapshot-json-reader-implementation.md`：Stage 27 filtered v3 实现与验收边界。
5. `tasks/handoff-2026-07-16.md`：Stage 32 收口事实与 Stage 33 条件启动边界。
6. `docs/88-filtered-snapshot-host-consumer-validation-gate.md`：Stage 28 consumer design gate。
7. `docs/86-filtered-envelope-snapshot-read-design-gate.md`：Stage 26 filter、identity 与关系闭包契约。
8. `docs/84-envelope-scoped-snapshot-read-design-gate.md`：Stage 23/24 envelope-scoped reader 边界。
9. `docs/83-codex-desktop-snapshot-json-reader-implementation.md`：Stage 22 project-scoped v1 reader。
10. `docs/79-read-only-host-consumer-validation-boundary.md`：Stage 18 reference consumer。
11. `docs/78-control-panel-host-integration-boundary.md`：Stage 17 stdio handoff。
12. `docs/76-read-only-control-panel-mvp.md`：Stage 16 静态只读 Control Panel。
13. `docs/75-cli-automation-contract-discovery.md`：CLI 自动化事实源。
14. `docs/52-minimal-orchestration-loop.md`：Stage 14 收口设计事实源。
15. `docs/51-backend-first-api-boundary.md`：Stage 13 资源/操作边界。
16. `docs/02-roadmap.md`：完整路线图。
17. `docs/archive/release-notes/93-release-notes-v0.14.0-filtered-snapshot-host-integration.md`：v0.14.0 验收事实。
18. `docs/archive/release-notes/92-release-notes-stage30-stage31-codex-desktop-filtered-snapshot-host-integration.md`：Stage 30/31 验收事实。
19. `docs/archive/77-read-only-control-plane-milestone-freeze.md`：v0.13.0 历史冻结事实源。
20. `docs/archive/81-codex-desktop-read-only-adapter-implementation.md`：Stage 20 历史实现。
21. `docs/archive/82-read-only-representation-read-design-gate.md`：Stage 21 历史 validation-only 设计门。
22. `docs/10-cli-poc-usage.md`：具体 CLI 参数。
...
当前状态：**Stage 32 — Filtered Snapshot Host Integration Milestone Freeze 已收口**；已实现固定 filtered v3 reader → Stage 29 consumer 的 validation-before-display one-shot host，并冻结本地 annotated tag `v0.14.0-filtered-snapshot-host-integration`。下一阶段为 **Stage 33 — Codex Desktop Filtered Snapshot Display Integration Gate（条件启动）**；通用 query、HTML/browser、live service、DB、auth、网络、文件 export 与 UI 写操作仍不开放。

- 冻结基线：`v0.14.0-filtered-snapshot-host-integration`（本地 tag，未 push）；上一基线为 `v0.13.0-read-only-control-plane` / `f401b98`。
...
- Stage 30–32 事实源：`docs/90-codex-desktop-filtered-snapshot-host-integration-and-milestone-freeze.md`；one-shot host 与 v0.14.0 本地里程碑已冻结。
