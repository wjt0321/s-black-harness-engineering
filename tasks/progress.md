...
- 用户明确授权按 docs/archive/110 实现 `pi_cli_print` 与一次受控真实 DeepSeek smoke。
...
## 2026-07-27 阶段 86 — Pi/OMP 真实只读状态接入（已完成并归档）

- 本机核验 Pi 0.82.0 与 OMP 1.3.14 的项目级扩展位置；用户确认 QwenPaw 为 2.0.1，旧虚拟环境 1.1.12.post3 不再作为当前事实源，QwenPaw 延后。
- 新增 `.pi/extensions/s-black-live-status.ts`、`.omp/extensions/s-black-live-status.ts` 与公共原子发布器；只监听宿主生命周期，不读取提示词、模型、工具内容或凭据，不访问网络、不启动子进程。
- 新增 `pi-local`、`omp-local` 固定生产绑定和内容摘要审阅；读取器只允许固定配置，中文控制面板新增“外部智能体 / 实时状态”。
- 已覆盖发布器开始/心跳/关闭、generation、单写者租约、绑定失败隔离、安全读取、缺席态和不可派发投影；Pi 项目扩展加载兼容验证通过。
- 真实验收通过：Pi/OMP 均显示“已连接，存在未绑定会话”，关闭后报告 `session_state=closed`、租约释放，并在 15 秒有效期后显示“状态已过期”；全程不可派发。
- 修复 OMP 不提供 Pi 项目信任方法的兼容问题；Pi 继续强制项目信任。
- 事实源与实施计划已移入 `docs/archive/` 和 `docs/archive/plans/`。
- 本轮没有提交或推送授权。
