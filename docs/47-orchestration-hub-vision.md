...
这个项目后续必须坚持**积木式（modular / pluggable）**扩展，而不是做成一个只能靠硬编码继续长大的大一统系统。

核心要求：

- 新的 Agent、工具、服务接入时，应以“新增积木”的方式插入，而不是重写主流程。
- 现有 guardrail、routing、state、report、approval 等层应尽量复用，不为单一工具定制一整套旁路。
- 上层 workflow 应优先依赖 capability 和统一状态对象，而不是直接写死某个脚本、runner 或平台接口。
- 未来增加 UI、自动化入口、后台服务时，也应消费同一套 control plane 抽象，而不是绕过后端内核。

可插拔不意味着一开始就追求过度抽象，而是要求：

> 每新增一类能力，都应该优先问“它能不能作为一个新积木插进现有结构”，而不是“要不要再开一条专用旁路”。
