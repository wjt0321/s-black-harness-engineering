<!-- parents: archive/110-pi-controlled-dry-run-adapter-contract.md -->
...
按 docs/archive/110 contract 实现唯一固定 operation：
...
- `agent_runtime/pi_print_runner.py`：复用 Stage 49 Windows Job Object containment（suspended spawn、`KILL_ON_JOB_CLOSE`、tree terminate/kill、no-orphan accounting），按 Pi 重定参数：每流 256 KiB、timeout 5..120s（默认 60s）、独立 duration bucket；不含 executable trust image 验证（docs/archive/110 §5 信任缺口已声明）；POSIX 仍 unavailable，无退化实现。
...
- npm/node 安装完整性是声明的信任缺口（docs/archive/110 §5）：v1 不是 trusted executable chain。
