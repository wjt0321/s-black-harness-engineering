...
下一阶段建议进入 **Stage 21 — Read-only Representation Read Design Gate**：先确认是否存在实际的 snapshot/HTML 消费需求，再单独设计用户显式授权、argv allowlist、输出脱敏和 no-write 证明；在新的 design gate 通过前，不扩展当前 adapter。
