...
前台窗口聚合既有 Pi/OMP 安全状态和阶段 89 有限链路的安全摘要；关闭窗口即停止轮询，不监听端口、不启动服务。阶段 91 GUI 增加“启动已登记链路”和最终人工决定：前者不要求输入链路 ID、任务、计划或目标，自动装配唯一已登记的验收配置和安全链路 ID；后者只选择“通过 / 要求修改”，链路自动完成后会直接路由到该页面。两者先生成既有一次性确认摘要，第二次点击才转发到既有阶段 89 operation；不提供任意命令、重试、取消、恢复、宿主控制或新的 CLI 执行参数。
...
参数严格有界：刷新间隔为 2–60 秒，链路摘要为 1–20 条。链路列表只显示 ID、状态、任务 ID、固定角色宿主和创建时间；确认页只展示 operation、链路 ID、绑定哈希和既有安全摘要，不复制原始目标、指令、未扫描原文、原始产物或任意项目文件内容。阶段 90 的只读 GUI 真实验收已于 2026-07-28 完成：窗口显示两个真实宿主、既有“通过 / 要求修改”链路摘要，并在关闭 OMP 后投影为“状态已过期”。阶段 91 已于 2026-07-28 在操作者实际打开、无工具、空闲的 Pi/OMP 上完成真实启动与最终决定 GUI 验收：固定链路自动完成 `Pi → OMP → Pi`，之后自动弹出最终人工决定，最终状态为 `approved`；fixture 只能覆盖投影和严格信封逻辑。
