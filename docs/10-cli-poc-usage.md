...
参数严格有界：刷新间隔为 2–60 秒，链路摘要为 1–20 条。链路列表只显示 ID、状态、任务 ID、固定角色宿主和创建时间；确认页只展示 operation、链路 ID、绑定哈希和既有安全摘要，不复制原始目标、指令、未扫描原文、原始产物或任意项目文件内容。阶段 90 的只读 GUI 真实验收已于 2026-07-28 完成：窗口显示两个真实宿主、既有“通过 / 要求修改”链路摘要，并在关闭 OMP 后投影为“状态已过期”。阶段 91 已于 2026-07-28 在操作者实际打开、无工具、空闲的 Pi/OMP 上完成真实启动与最终决定 GUI 验收：固定链路自动完成 `Pi → OMP → Pi`，之后自动弹出最终人工决定，最终状态为 `approved`。阶段 92 已于 2026-07-28 完成正向与反向真实 Pi/OMP GUI 验收：操作者仅选择工作卡、确认启动和最终决定；确认页执行中保持可刷新，启动前与提交前会只读预检全部涉及宿主。fixture 仅覆盖投影、固定收件箱和严格信封逻辑。
