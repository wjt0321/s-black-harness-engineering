...
- `report`: Report Preview，`status=preview`，包含 candidate event/artifact/evidence 计数与类型分布、`requires_approval`、`next_action`、仅 rule_ids 的 finding 摘要；无 `report_id`。Evidence 只暴露候选数量与 `evidence_type` 分布，不回显正文或敏感值。
