...
<!-- relates: 60-orchestration-run-lifecycle-events-design.md, 95-single-user-real-execution-readiness-gate-and-milestone.md -->

# 97 — Execution Lifecycle Audit Writer Design and Implementation

> 状态：**Stage 47 design gate 已冻结；Stage 48 implementation 待按 TDD 验收**
> 日期：2026-07-17
...
3. started 与 terminal 各自按原始 byte size 原子追加并在失败时回滚本次写入；
...
`plan_hash` 与 output digest 使用 `sha256:<64 lowercase hex>`。id 字段、adapter/capability/operation/failure code 使用长度受限的 ASCII token。schema 和 writer 都不得接受额外 metadata。
...
6. 记录 events ledger 原始 byte size；
7. 追加一行并 flush；
8. 对真实 ledger 运行 event schema、task/event consistency 与 execution audit consistency；
9. post-check 失败时 truncate 到原始 byte size；
10. 成功后返回只含 safe ids/status 的 result。
...
terminal 也按单行 byte-size rollback。若 terminal write 或 post-check 失败，只回滚 terminal append；已提交的 started 保留。result 返回 `audit_incomplete=true`，future executor 必须 withheld execution result，并通过 recovery 路径重试 terminal audit 或人工处置。
...
输出只包含 attempt/event/task/request/plan identity、terminal type、phase 与 safe recovery action，不复制任意 raw metadata value。
...
6. started write/post-check failure byte-size rollback；
...
14. controlled-write regression 覆盖通用入口不可伪造与专用 writer append/rollback；
15. source 不导入 subprocess/socket/requests，不执行外部命令。
...
<!-- stage48-implementation-status: pending -->
