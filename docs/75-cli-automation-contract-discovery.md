...
## 第五拍：Workflow Plan Re-check / Drift Validation

### 方案选择

阶段收口只实现 hash-based re-check：调用方保存上一轮 `plan_id`，随后以同一 `profile_id` 重新投影当前 plan，并比较内容哈希。

对比过但不采用的方案：

- 读取任意旧 plan 文件：可以做字段级 diff，但会扩大文件读取面和输入校验面；当前不需要。
- 持久化 plan registry/ledger：可以保留历史，但会把 ephemeral read model 升级为新存储；明确越过本阶段边界。

因此 v1 只可靠声明 projection 是否发生 drift；hash 不匹配时不猜测具体字段原因。输出会携带当前完整 plan，调用方若保存过旧 payload，可自行做字段级比较。
...
输出 schema：`control-plane/automation-workflow-check/v1`。

- expected id 必须严格匹配 `sha256:[a-f0-9]{64}`；非法格式返回 `needs_input`，不回显原始非法值，也不读取 profile registry；
- 当前 plan 无法生成时，原样传播 profile/contract gate 的 `blocked`、`needs_input` 或 `validation_failed`，不伪造 drift 结论；
- hash 相同返回 `pass`、`matches_current=true`、`drift.detected=false`；
- hash 不同返回 `blocked`、`matches_current=false`、`drift.detected=true` 与 `automation-workflow-plan-drift` finding；
- drift 只表示 canonical workflow projection 已变化，`field_level_diff_available=false`；
- check 仍然不执行 current plan 中的任何 command，不写文件/ledger，不访问网络。

manifest 增加 `automation_workflow_check` preview/read-only 条目，关键 flag 为 `--profile-id` 与 `--expected-plan-id`。

## 本轮阶段收口标准

post-Stage 14 CLI 自动化消费者在以下条件全部满足后收口：

1. contract discovery、requirement gate、source-backed profile、workflow plan、plan re-check 五层契约均有版本化 JSON projection；
2. available command path 与关键 flag 由 argparse 反向契约测试冻结；
3. stable/preview/access 边界可机器判断，preview 不被静默升级；
4. workflow plan 与 re-check 保持 deterministic、no-write、no-network、no-command/adaptor execution；
5. unknown、unavailable、access exceeded、invalid id、drift 等失败路径有结构化状态、finding/next action 与退出码；
6. 全量测试、doctor、public scan、controlled-write regression、compileall、docs context、diff check 与 pre-commit 全部通过；
7. release notes、stage digest 与 handoff 明确“本消费者阶段已收口，下一产品阶段不自动启动”。
